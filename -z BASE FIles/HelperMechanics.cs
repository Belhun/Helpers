...
namespace Helpers
{
    public static class HelperMechanics
    {
        ...
        //public static float CalculateTendSpeedContribution(Pawn doctor, List<Pawn> helpers)
        //{
        //    float totalContribution = 0f;
        //    foreach (var helper in helpers)
        //    {
        //        int medicineSkill = helper.skills.GetSkill(SkillDefOf.Medicine).Level;
        //        float contribution = 0.5f + (medicineSkill / 40f); // Base 50%, scales with skill

        //        if (medicineSkill < 5)
        //            contribution -= 0.1f; // Penalize low-skill helpers

        //        totalContribution += contribution * helper.GetStatValue(StatDefOf.MedicalTendSpeed);
        //    }
